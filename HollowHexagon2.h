...
#ifndef HOLLOW_HEXAGON2_H
#define HOLLOW_HEXAGON2_H

#include "Mesh.h"

class HollowHexagon2 {
public:
	static Mesh init() {
		Mesh mesh;
...
private:
	static void vertices(Mesh& mesh) {
		...
		mesh.vertex({ 0.204f, -0.021f, -0.047f });
		mesh.vertex({ 0.115f, -0.029f, 0.149f });
		mesh.vertex({ -0.110f, 0.014f, 0.222f });
		mesh.vertex({ -0.204f, 0.049f, 0.049f });
		mesh.vertex({ -0.126f, -0.043f, -0.207f });
		mesh.vertex({ 0.134f, 0.010f, -0.131f });
		...
		mesh.vertex({ 0.158f, 1.091f, -0.020f });
		mesh.vertex({ 0.140f, 1.120f, 0.195f });
		mesh.vertex({ -0.140f, 1.121f, 0.207f });
		mesh.vertex({ -0.198f, 1.164f, 0.038f });
		mesh.vertex({ -0.113f, 1.070f, -0.147f });
		mesh.vertex({ 0.063f, 1.071f, -0.150f });
