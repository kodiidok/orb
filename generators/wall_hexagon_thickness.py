...
def semi_circle(cx, cy, cz, radius, segments, ratio):
    v = []
...
for j in range(2):
    ...
    for i in range(6):
        ...
        angle_offset = 60.0 * i
        translation_vector = [hexRadius[j] * math.sin(math.radians(angle_offset)), 1.0, hexRadius[j] * math.cos(math.radians(angle_offset))]
        
        # Generate semi-circle
        semi_circle_vertices = semi_circle(0.0, 0.0, 0.0, radius, segments, ratio[j])
