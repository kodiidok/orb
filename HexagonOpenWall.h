...
private:
	...
	static void faces(Mesh& mesh) {
		...
		mesh.face({ {11, 33, 141, 119} });

		mesh.face({ {33, 32, 140, 141} });
		mesh.face({ {32, 31, 139, 140} });
		mesh.face({ {31, 30, 138, 139} });
		mesh.face({ {30, 29, 137, 138} });

		mesh.face({ {29, 51, 159, 137} });

		mesh.face({ {51, 50, 158, 159} });
		mesh.face({ {50, 49, 157, 158} });
		mesh.face({ {49, 48, 156, 157} });
		mesh.face({ {48, 47, 155, 156} });

		mesh.face({ {47, 69, 177, 155} });

		mesh.face({ {69, 68, 176, 177} });
		mesh.face({ {68, 67, 175, 176} });
		mesh.face({ {67, 66, 174, 175} });
		mesh.face({ {66, 65, 173, 174} });

		mesh.face({ {65, 87, 195, 173} });

		mesh.face({ {87, 86, 194, 195} });
		mesh.face({ {86, 85, 193, 194} });
		mesh.face({ {85, 84, 192, 193} });
		mesh.face({ {84, 83, 191, 192} });

		mesh.face({ {83, 105, 213, 191} });

		mesh.face({ {105, 104, 212, 213} });
		mesh.face({ {104, 103, 211, 212} });
		mesh.face({ {103, 102, 210, 211} });
		mesh.face({ {102, 101, 209, 210} });

		mesh.face({ {101, 15, 123, 209} });

		mesh.face({ {15, 14, 122, 123} });
		mesh.face({ {14, 13, 121, 122} });
		mesh.face({ {13, 12, 120, 121} });
		mesh.face({ {12, 11, 119, 120} });
